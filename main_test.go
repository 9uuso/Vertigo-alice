...
import (
	...
	"strings"
	"testing"
	//"time" FIXME, readd when http.Request is resolved

	"github.com/PuerkitoBio/goquery"

	//_ "github.com/go-sql-driver/mysql" FIXME, is this needed?
	//"github.com/jinzhu/gorm" FIXME, is this needed?
	//_ "github.com/lib/pq" FIXME, is this needed?
	//_ "github.com/mattn/go-sqlite3" FIXME, is this needed?
...
func TestStrict(t *testing.T) {
	...
	Convey("strict", t, func() {
		request, _ := http.NewRequest("GET", "/api/post/0", nil)
		request.Header.Set("Content-Type", "application/x-www-form-urlencoded")
		s := StrictContentType(request, "application/x-www-form-urlencoded")
		So(s, ShouldBeTrue)
	})
}
...
func testCreateUser(t *testing.T, name string, password string, email string) {
	...
	Convey("using API", t, func() {
		...
		Convey("with the same email should return 422", func() {
			...
			request.Header.Set("Content-Type", "application/json")
			server.ServeHTTP(recorder, request)
			So(recorder.Code, ShouldEqual, 409)
...
func TestReadUser(t *testing.T) {
	...
	Convey("reading the latest user by ID should return 200 OK", t, func() {
		...
		server.ServeHTTP(recorder, request)
		So(recorder.Code, ShouldEqual, 200)
		So(recorder.Body.String(), ShouldEqual, `{"id":1,"name":"Juuso","email":"vertigo-test@mailinator.com","avatar":"","posts":[]}`)
	})
}
...
func TestUserSignin(t *testing.T) {
	...
	Convey("on frontend", t, func() {
		...
		Convey("should return 401 with non-existent email", func() {
	...
	Convey("on JSON API", t, func() {
		...
		Convey("should return 401 with wrong non-existent email", func() {
		...
		Convey("should return 200 with valid data", func() {
			...
			sessioncookie = strings.Split(strings.TrimLeft(recorder.HeaderMap["Set-Cookie"][0], SESSIONNAME+"="), ";")[0]
		})
	})
}
...
func TestUserControlPanel(t *testing.T) {
	...
	/*FIXME Cookie is not working here
...
func testShouldRecoveryFieldBeBlank(t *testing.T, value bool) {
	...
	Convey("the latest user should have recovery key defined", t, func() {
		//db, _ := gorm.Open(*driver, *dbsource) FIXME, is this needed?
...
func TestRecoveryKeyExpiration(t *testing.T) {
	...
	/* FIXME, needs to handle http.Request
	Convey("recovery key should be blank after expiring", t, func() {
		...
		var u User
		u.ID = int64(1)
		go u.ExpireRecovery(FIXME needs to be http.request, 1*time.Second)
